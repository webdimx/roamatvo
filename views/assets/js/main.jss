$(document).ready(function(){
	...
	$('.removeSell').click(function(){
		
		
		$.post(ajaxUrl+"/transacoes/cancel", {'ID': $(this).attr('data-id')}, function(){
	...
	$('.gswap').click(function(){
			...
			($(this).data('tipo')==1? $('.swapT').show():$('.swapT').hide())
			...
			$.post(ajaxUrl+'swap/getFornecedores', {tipo: $(this).data('tipo'), adiar: $('[name=adiar]:checked').val()}, function(a){
		...
		$('.makeSwap').click(function(){
			...
			$.post(ajaxUrl+'swap/getSwapPendent', {tipo: $(this).data('tipo'), 'fornecedores': JSON.stringify($f), nextDay: $nd}, function(a){
