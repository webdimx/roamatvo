...
class TransacoesModel extends MainController
{
	...
	public function _submit(){
		...
		if(chk_array( $this->form_data, 'emitir_nota')!=2): $this->gerFatura($last_id); endif;
	...
	public function getList($yes, $helpdesk, $reembolso, $cancelamento, $prorrogacao,  $controle) {
	...
	public function setStatus($id, $status, $novo){
		...
		$query = $this->db->update($this->tableSIM, 'simcard', $id, array(
		...
		if($Mstatus):
		$query = $this->db->update($this->tableMdn, 'ID', $data["id_associacao"], array(
	...
	public function setStatusOld($id){
		...
		$query = $this->db->update($this->tableSIM, 'ID', 10, array(
	...
	public function associate($mdn, $simcard){
		...
		if(!$query->fetch()):
		    ...
		    if($dataSim):
				...
				$query = $this->db->update($this->tableMdn, 'ID', $dataSim["id_associacao"], array(
			...
			$query = $this->db->update($this->tableSIM, 'simcard', $simcard, array(
	...
	public function cancel($id){
		...
		$query = $this->db->update($this->table, 'ID', $id, array(
	...
	public function importerOziel($data){
		...
		$query = $this->db->insert('wd_importer', array(
	...
	public function exportReport($id, $yes, $controle){
		...
		'local_uso' => "j.local as 'Local de Uso'",
		'dias_uso' => "a.dias_uso as 'Dias de Uso'",
		'valor_plano' => "a.valor_plano as 'Valor do Plano'",'valor_plano' => "b.valor as 'Valor do Plano'",
	...
	function getMdn(){
		...
		$query = $this->db->update($this->table, 'ID', $_data[ID], array(
	...
	public function recharge(){
		...
		if($data):
				...
				$this->db->insert('wd_recargas', array(
