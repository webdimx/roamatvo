...
<div id="dynamic-table_wrapper" class="dataTables_wrapper form-inline no-footer">
	...
	<div class="dt-buttons btn-overlap btn-group">
		<div class="btn-group btn-corner">
			...
			if (!$this->subController == 'historico'):
				?>
				<a href="<?= HOME_URI . $this->controller ?>?mdn=1" class="btn btn-primary btn-sm  clear-filters"><i
						class="fa fa-files-o bigger-110 "></i> Apenas MDN</a>
				<a href="<?= HOME_URI . $this->controller ?>?simcard=1" class="btn btn-primary btn-sm  clear-filters"><i
						class="fa fa-files-o bigger-110 "></i> Apenas SIMCARD</a>
				<a href="<?= HOME_URI . $this->controller ?>/historico" class="btn btn-primary btn-sm "><i class="fa fa-history"
						aria-hidden="true"></i> Histórico</a>
				<a href="<?= HOME_URI . $this->controller ?>?transacao=1" class="btn btn-primary btn-sm  clear-filters"><i
						class="fa fa-files-o bigger-110 "></i> Associados a Transações</a>
				<?
			endif;
			?>
			<a href="#" class="btn btn-primary btn-sm btn-success export" data-action="all"><i
	...
	<table id="dynamic-table"
		class="table table-striped table-bordered table-hover dataTable no-footer table-<?= $this->controller ?>"
			...
			<tr role="row">
				...
				<? if (!$_GET[mdn]): ?>
					<th rowspan="1" colspan="1">SIMCARD<br><input type="text" class="filter" name="simcard"
							value="<?= $_GET['simcard'] ?>"><input type="hidden" value="<?= $_GET[historico] ?>" name="historico"> </th>
				<? endif; ?>
				<? if (!$_GET[simcard]): ?>
					<th rowspan="1" colspan="1">MDN<br><input type="text" class="filter" name="a|mdn" value="<?= $_GET['a|mdn'] ?>">
					</th>
				<? endif; ?>
				<? if (!$_GET[simcard]): ?>
					<th rowspan="1" colspan="1">Status MDN<br>
						<select name="status_mdn" class="filter">
							<option value=""></option>
							<?
							foreach ($modelConfig->getOptions('status_mdn') as $option):
								?>
								<option value="<?= $option['ID'] ?>" <?= ($option['ID'] == $_GET[status_mdn] ? 'selected' : '') ?>>
									<?= $option['status'] ?>
								</option>
								<?
							endforeach;
				...
				<? endif; ?>
				<? if (!$_GET[mdn]): ?>
					<th rowspan="1" colspan="1">Estoque<br>
				...
				<th rowspan="1" colspan="1">Ativado em<br><input type="text" class="filter date-range date "
				...
				<th rowspan="1" colspan="1">Data Off<br><input type="text" class="filter date-range date "
				...
				<th rowspan="1" colspan="1">Fornecedores<br>
					<select name="fornecedor" class="filter">
						...
						foreach ($modelConfig->getListAssociate() as $option):
							?>
							<option value="<?= $option[value] ?>" <?= ($option[value] == $_GET['fornecedor'] ? 'selected' : '') ?>>
								<?= $option[fornecedor] ?>
							</option>
							<?
						endforeach;
			...
			foreach ($_attributes[data] as $_item):
				...
				<tr role="row" class="odd">
					...
					<td>
						...
						<?= ($_item[fornecedor_simcard] && $_item[fornecedor_mdn] ? '/' : '') ?>
