...
<div class="btn-group btn-corner">
    <a href="<?=HOME_URI.$this->controller?>/adicionar-simcard" class="btn btn-success btn-sm filterRegistry filterRegistry"><i class="fa fa-plus bigger-110 "></i> Novo</a>
    
    <?
    if($_SESSION['userdata']['grupo']!=8):
    ?>
	<a href="#modal-import-lote" class="btn btn-success btn-sm ic" data-toggle="modal" data-type="2"><i class="fa  fa-cloud-upload bigger-110 "></i> Importar Lote SIMCARD</a>
	<a href="#" class="btn btn-warning btn-sm exportCad" data-type="simcard" data-method="all"><i class="fa fa-file-excel-o bigger-110 "></i> Exportar</a>
	<a href="#" class="btn btn-warning btn-sm exportCad" data-type="simcard"><i class="fa fa-file-excel-o bigger-110 "></i> Exportar Selecionados</a>
	<?
	endif;
	?>
...
<table id="dynamic-table" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dynamic-table_info">
      <thead>
		  ...
		  <select  name="fornecedor_simcard"  class="filter">
					...
					?>
					<option value="<?=$option[ID]?>" <?=($option[ID]==$_GET[fornecedor_simcard]?'selected':'')?>><?=$option[nome]?></option>
					<?
					 endforeach;  
					?>
			</select>   	  
		  ...
		  <th rowspan="1" width="100" colspan="1">Data<br><input class="form-control date-range date filter" name="data" value="<?=$_GET['data']?>"   type="text" data-date-format="dd/mm/yyyy"></th>
      ...
      <tbody>
          ...
          <tr role="row" class="odd">
              ...
              <td>
				  ...
				  <a href="<?=HOME_URI?>transacoes/?iccid=<?=$_item[simcard]?>"><?=$_item[simcard]?></a>
			...
			<?=($_item[tipo_uso]==3?'<button class="btn btn-minier bigger btn-primary" data-rel="tooltip" title="Casado com o MDN '.$_item[mdn].'">C</button>':'')?>  </td>
			  ...
			  <td align="center">
				  <a href="<?=HOME_URI.$this->controller."/editar-simcard/".$_item[ID]?>"><button class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button></a>
			  	  <a href="#"  data-rel="tooltip" data-toggle="modal" title="Excluir" class="delSim" data-id="<?=$_item[simcard]?>"><button class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></button></a>
			  </td>
		  </tr>              
          <?
          endforeach;
