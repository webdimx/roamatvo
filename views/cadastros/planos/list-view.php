...
<div class="btn-group btn-corner">
    <a href="<?=HOME_URI.$this->controller?>/adicionar-plano" class="btn btn-success btn-sm filterRegistry filterRegistry"><i class="fa fa-plus bigger-110 "></i> Novo</a>
    ...
    <a href="#modal-confirm" data-toggle="modal" data-table="wd_planos" class="btn btn-danger  btn-sm  delete "><i class="fa fa-trash-o "></i> Excluir selecionados</a>
</div>
</div>
...
<table id="dynamic-table" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dynamic-table_info">
      <thead>
          ...
          <th rowspan="1" colspan="1">Nome<br><input type="text" class="filter" name=a|nome value="<?=$_GET['a|nome']?>"></th>
          ...
          <th rowspan="1" colspan="1">Valor(USD)<br><input type="text" class="filter moneyUSD" name="valor" value="<?=$_GET[valor]?>"></th>
          <th rowspan="1" colspan="1">Qtd de Dias<br><input type="text" class="filter"  name="qtd_dias" value="<?=$_GET['qtd_dias']?>"></th>
          <th rowspan="1" colspan="1">Local de Uso<br>
			<select  name="local_uso"  class="filter">
					<option value=""></option>
				    <?
					 foreach($modelConfig->getOptions('local_de_uso', true) as $option):  
					?>
					<option value="<?=$option[ID]?>" <?=($option[ID]==$_GET[local_uso]?'selected':'') ?> > <?=$option[local]?></option>
					<?
					 endforeach;  
					?>
			</select>	  
		  ...
		  <th rowspan="1" colspan="1">Fornecedor Indicado<br>
			<select  name="fornecedor"  class="filter">
					...
					?>
					<option value="<?=$option[ID]?>" <?=($option[ID]==$_GET[fornecedor]?'selected':'') ?> > <?=$option[nome]?></option>
					<?
					 endforeach;  
					?>
			</select>	  
      ...
      <tbody>
          ...
          <tr role="row" class="odd">
              ...
              <td><?=($_item[observacao]?'<button class="btn btn-xs btn-warning" data-rel="tooltip" data-placement="left" title="'.$_item[observacao].'"><i class="ace-icon fa fa-flag bigger-120"></i></button>':'')?>
			  </td>
              <td align="center"><a href="<?=HOME_URI.$this->controller."/editar-plano/".$_item[ID]?>"><button class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button></a></td>
		  </tr>
          <?
          endforeach;
          ?>
