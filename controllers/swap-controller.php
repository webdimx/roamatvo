...
class SwapController extends MainController
{
		...
		if ($itens):
			...
			<div class="profile-user-info profile-user-info-striped">
				...
				foreach ($itens as $data):
					...
					<div class="profile-info-row">
						<div class="profile-info-name" style="width: 25px">
							<label class="pos-rel"><input type="checkbox" class="ace" value="'<?= $data[ID] ?>'"><span
			...
			?>
			<p></p>
			<p class="text-center">Não existe Swap de
				<?= ($_POST[tipo] == 1 ? 'ativação' : 'desativação') ?> pendente para hoje!
			</p>
			<?
		endif;
	}
	...
	public function export()
	{
		...
		if ($data[0][tipo] != 2):
			$spreadsheet->setActiveSheetIndex(0)->setCellValue('B1', 'SIMCARD ANTIGO');
		endif;
		if ($data[0][tipo] != 2):
			$spreadsheet->setActiveSheetIndex(0)->setCellValue('C1', 'ÚLTIMA ' . ($data[0][tipo] == 1 ? 'DESATIVAÇÃO ' : 'DESATIVAÇÃO ') . '');
		endif;
		...
		foreach ($_attributes as $_item):
			...
			if ($data[0][tipo] != 2):
				$spreadsheet->setActiveSheetIndex(0)->setCellValue('B' . $x, '="' . $last[iccid] . '"');
			endif;
			if ($data[0][tipo] != 2):
				$spreadsheet->setActiveSheetIndex(0)->setCellValue('C' . $x, $last[data_off]);
			endif;
