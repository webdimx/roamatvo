                ...
                <p>
                  ...
                  <select  name="<?=$this->controller?>[fornecedor_mdn]"  class="col-xs-12 col-sm-12 col-lg-12 required <?=($data[fornecedor_mdn]?'':'') ?>">
					...
					<?
					 foreach($modelConfig->getOptions('fornecedores', true) as $option):
					?>
					<option value="<?=$option[ID]?>" <?=($option[ID]==$data[fornecedor_mdn]?'selected':($data[fornecedor_mdn]?'':'')) ?> > <?=$option[nome]?></option>
					<?
					 endforeach;
					?>
				  </select>
	...
	$(document).ready(function(){
		...
		$('#plano').change(function(){
			...
			$.post(ajaxUrl+'cadastros/getPlano', {ID: $(this).val()}, function(a){
				...
				$b = new Date($a[2], $a[1]-1, parseInt($a[0])+(parseInt(a[0].qtd_dias)-1), 0, 0, 0)
