...
class TransacoesModel extends MainController
{
	...
	public function getRegistry($id)
	{
		...
		$query = $this->db->query("SELECT a.*, b.ID as plano   FROM `" . $this->table . "` a LEFT JOIN wd_planos b on a.plano = b.codigo_plano   where a.ID = '" . $id . "' ");
		if (!$query) {
			return array();
		}
	...
	public function getPlan()
	{
		...
		$query = $this->db->query("select codigo_plano from wd_planos where id = '" . chk_array($this->form_data, 'planos') . "' ");
		$data = $query->fetch();
		return $data['codigo_plano'];
	...
	public function setTransaction($data)
	{
		...
		$sellData = [
			...
			'dias_uso' => $data->days,
			'valor_pago' => substr_replace($data->amount, '.', 2, 0),
			'email' => $data->email,
			'celular' => $data->phone,
			'local_venda' => $data->sales_location,
			'ponto_venda' => $data->sales_location,
			'origem' => 2,
			'fornecedor_simcard' => 5,
			'fornecedor_mdn' => 5,
