...
class AjaxModel
{
	...
	public function getSells($modelSwap, $modelEmail){
		...
		if(destino='canada-mexico', concat('CANMEX', periodo), if(destino='eua', concat('EUA', periodo), if(destino='latina', concat('AMI', periodo), if(destino='europe', concat('EUROPA', periodo), concat('MUNDO', periodo))))) as plano,
		a.nome,
		voucher_id_purchase,
		f.nome as retirado,
		telefone as celular,
		...
		valor_produto,
		REPLACE(format(REPLACE(if(valor_final_recebido!='0,00', replace(valor_final_recebido, '.', ''), voucher_valor_total), ',', '.'), 2, 'en_US'), ',', '') as valor_pago,
		valor_desconto,
		REPLACE(REPLACE(cotacao_dolar, 'R$', ''), ',','.') AS valor_dolar_dia,
		REPLACE(REPLACE(cotacao_dolar, 'R$', ''), ',','.') AS valor_dolar_dia,
		REPLACE(REPLACE(valor_recebido_dolar, '.', ''), ',','.') valor_dolar,
		REPLACE(REPLACE(valor_recebido_euro, '.', ''), ',','.') valor_euro,
		REPLACE(REPLACE(valor_recebido_real, '.', ''), ',','.') valor_real,
		REPLACE(REPLACE(valor_recebido_debito, '.', ''), ',','.') valor_debito,
		REPLACE(REPLACE(valor_recebido_credito, '.', ''), ',','.') valor_credito,
		cpf,
		cnpj,
		cortesia,
		...
		$querySim = $this->db->query("SELECT * FROM `wd_simcards` where upper(simcard) = '$item[iccid]'");
		if ( $querySim ) {
		...
		if($dataSim[fornecedor]==46){


		   $query4 = $this->db->query("SELECT *, fornecedor AS fornecedor_mdn, fornecedor AS fornecedor_simcard,  fornecedor AS fornecedor_mdn FROM wd_mdns WHERE fornecedor = 46 AND status = 1");
	  ...
	  /*	if($item[plano]=='EUA10' || $item[plano]=='EUA05'):
		...
		*/
		...
		$query = $this->db->insert('wd_transacoes', array(
				...
				'data_off' => getDataOff($item[data_ativacao], $item[dias_uso]),
				'valor_plano' => formatPayment(($item[valor_produto]?$item[valor_produto]:0.00)),
				'forma_pagamento' => $item[forma_pagamento],
				'moeda' => $item[moeda],
				'desconto' => ($item[valor_desconto]?formatMoney($item[valor_desconto]):0.00),
				'valor_pago' => formatMoney(($item[valor_pago]?str_replace('.',',', $item[valor_pago]):0.00)),
				'valor_dolar_dia' => $item[valor_dolar_dia],
				'valor_credito' => $item['valor_credito'],
				'valor_debito' => $item['valor_debito'],
				'valor_real' => $item['valor_real'],
				'valor_dolar' => $item['valor_dolar'],
				'valor_euro' => $item['valor_euro'],
				...
				'origem' => (($getOrigin?$getOrigin:$data5[sigla])=='Matriz'?'5':$origem),
				'emitir_nota' => ($item[cpf_nota]?'1':0),
				'detalhe' => (($getOrigin?$getOrigin:$data5[sigla])=='Matriz'?'HD':$data5[sigla]),
				'tipo_transacao' => ($data4[tipo_transacao]?$data4[tipo_transacao]:2),
				'ocorrencia' => ($item[cortesia]>'0'?3:'1')
	...
	public function getCorpFix($v){
		...
		$query1 = $t1->query("SELECT f.nome as retirado, a.codigo_chip, d.nome as aeroporto, upper(trim(SUBSTRING_INDEX(observacoes, '-', 1))) as voucher, if(id_quiosque!=9999, b.nome  , id_quiosque) as local_venda, a.id  FROM vendas a LEFT JOIN quiosques b on a.id_quiosque = b.id LEFT JOIN aeroportos d on b.id_aeroporto = d.id  left join quiosques f on a.voucher_retirado_quiosque_id = f.id where a.id > '59000' ");
		...
		echo "SELECT $data[codigo_chip] FROM `wd_ponto_de_venda` where ".($data[local_venda]!='9999'?"ponto = '$data[local_venda]'":"ponto = '$data[retirado]'")."<br> ";
		...
		$query2 = $this->db->query("SELECT * FROM `wd_ponto_de_venda` where ".($data[local_venda]!='9999'?"ponto = '$data[local_venda]'":"ponto = '$data[retirado]'")." ");
		if ( $query2 ):
		$data2 = $query2->fetch();
		...
		$query = $this->db->update('wd_transacoes', 'id_skillsim', $data[id], array('ponto_venda' => $data2[ID]));
	...
	public function getOptionsRecharge(){
		...
		$query = $this->db->query("SELECT * from wd_planos where local_uso = (SELECT b.local_uso FROM wd_transacoes a LEFT JOIN wd_planos b ON a.plano = b.ID where a.iccid = '".$_POST[iccid]."' AND a.status = 2)  AND qtd_dias <= 30 ORDER by qtd_dias");
