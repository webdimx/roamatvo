...
class UserLogin
{
	...
	public function check_userlogin()
	{
		...
		$query = $this->db->query(
			'SELECT a.*, b.permissoes as user_permissions, IF(a.entity_id=0,a.name, c.nome) as name  FROM users a LEFT JOIN wd_grupo_de_acesso b ON a.grupo = b.ID LEFT JOIN wd_colaboradores c on a.entity_id = c.ID WHERE a.email = ? LIMIT 1',
			array($email)
		);
		...
		if (empty($user_id)) {
			$this->logged_in = false;
			$this->login_error = 'O login ou senha não existe!';
		...
		if ($fetch['status'] != 1) {
			$this->logged_in = false;
			$this->login_error = 'Acesso não permitido!';
		...
		if ($this->phpass->CheckPassword($user_password, $fetch['user_password'])) {
			...
			// A senha não bateu
			$this->login_error = 'A senha não confere.';
	 ...
	 */
	protected function goto_login($advice = NULL)
	{
		...
		if (defined('HOME_URI')) {
			...
			$login_uri = HOME_URI . 'login/' . $advice;
