...
class CmovelModel extends MainController
{
	...
	public function active(){
		...
		$query = $this->db->query("SELECT a.ID, iccid, codigocmovel, if((fornecedor_simcard = 48 && fornecedor_mdn = 48) || LOCATE('eua', lower(b.nome)), 1, 0) AS exception FROM ".$this->table." a LEFT JOIN ".$this->tablePlanos." b ON a.plano = b.ID WHERE ((fornecedor_mdn = 40 AND fornecedor_simcard = 40) || (fornecedor_mdn = 48 AND fornecedor_simcard = 48)) AND STATUS = '1' AND datediff(NOW(), data_ativacao) = 0 ORDER BY a.ID DESC");
