...
<div class="btn-group btn-corner">
		<a href="<?= HOME_URI . $this->controller ?>/adicionar-ponto-de-venda" class="btn btn-success btn-sm filterRegistry filterRegistry"><i class="fa fa-plus bigger-110 "></i> Novo</a>
	<a href="#" class="btn btn-primary btn-sm filterRegistry filterRegistry"><i class="fa fa-search bigger-110 "></i> Filtrar</a>
		...
		<a href="#modal-confirm" data-toggle="modal" data-table="wd_ponto_de_venda" class="btn btn-danger  btn-sm  delete "><i class="fa fa-trash-o "></i> Excluir selecionados</a>
</div>
</div>
<table id="dynamic-table" class="table table-striped table-bordered table-hover dataTable no-footer" role="grid" aria-describedby="dynamic-table_info">
			...
			<th rowspan="1" colspan="1">Ponto<br><input type="text" class="filter" name="a|ponto" value="<?= $_GET['a|ponto'] ?>"></th>

				<th rowspan="1" colspan="1">Local<br>
				<select  name="a|local"  class="filter">
					<option value=""></option>
						<?
						foreach ($modelConfig->getOptions('local_de_venda', true) as $option):
							?>
						<option value="<?= $option['ID'] ?>" <?= ($option['ID'] == $_GET["a|local"] ? 'selected' : '') ?> > <?= $option[local] ?></option>
						<?
						endforeach;
						?>
				</select>
				...
				<option value=""></option>
				<option value="1" <?= ($_GET["a|situacao"] == 1 ? 'selected' : '') ?>>Ativo</option>
				<option value="2" <?= ($_GET["a|situacao"] == 2 ? 'selected' : '') ?>>Inativo</option>
			</select>
			</th>
			...
			<tbody>
					...
					<?
					$_attributes = $model->getList('pontodevenda');
					...
					<td align="center"><a href="<?= HOME_URI . $this->controller . "/editar-ponto-de-venda/" . $_item[ID] ?>"><button class="btn btn-xs btn-info"><i class="ace-icon fa fa-pencil bigger-120"></i></button></a></td>
				</tr>
						<?
					endforeach;
					?>
